...
public interface FileSystemRepository extends JpaRepository<FileSystem, String> {
	...
	@Query(value = "SELECT * FROM filesystem WHERE parent_id = ? AND name = ? ", nativeQuery = true)
	List<FileSystem> getChild(@Param("file_id") String parentID
            , @Param("name") String childName);
